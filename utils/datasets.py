...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            # Load mosaic
            img, labels = load_mosaic(self, index)# 自带数据增强(这个马赛克拼接function在最下面）
            shapes = None
            ...
            if random.random() < hyp['mixup']:
                ...
                labels = np.concatenate((labels, labels2), 0) # 直接把label cat就行？ 不需要计算权重？
...
def load_mosaic(self, index):
    ...
    # 前面拼接成的是大图，需要增加到网络输入要求大小，随机平移、旋转和缩放增强操作
    # Augment
